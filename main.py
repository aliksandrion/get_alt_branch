...
class BaseAltAPI:
    def __init__(self):
        self.url = 'https://rdb.altlinux.org/api/export/branch_binary_packages'

    def get_branches_packages(self, first_branch, second_branch, arch=None):
        try:
            first_branch_response = get(f"{self.url}/{first_branch}", params={'arch': arch}).json()
            second_branch_response = get(f"{self.url}/{second_branch}", params={'arch': arch}).json()
    ...
    def __unique_packages(self, first_branch, second_branch, arch=None):
        compared_branches = self.get_branches_packages(first_branch, second_branch, arch)
    ...
    def first_branch_unique_packages(self, first_branch, second_branch, arch=None):
        return self.__unique_packages(first_branch, second_branch, arch)

    def second_branch_unique_packages(self, first_branch, second_branch, arch=None):
        return self.__unique_packages(second_branch, first_branch, arch)

    def unique_version_release(self, first_branch, second_branch, arch=None):
        compared_branches = self.get_branches_packages(first_branch, second_branch, arch)
